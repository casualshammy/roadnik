...
public class LogMiddleware
{
  ...
  private readonly Ax.Fw.SharedTypes.Interfaces.ILogger p_log;
  private long p_reqCount = -1;

  public LogMiddleware(
    RequestDelegate _next,
    Ax.Fw.SharedTypes.Interfaces.ILogger _log)
  {
    p_next = _next;
    p_log = _log;
  }
  ...
  public async Task Invoke(HttpContext _context)
  {
    ...
    p_log.Info($"[{reqIndex}] --> {request.Method} {request.Path}");
    await p_next(_context);
    p_log.Info($"[{reqIndex}] <-- {request.Method} {request.Path} {_context.Response.StatusCode}");
  }
}
