...
export class TimeSpan {
    ...
    private static interval(value: number, scale: number): TimeSpan | never {
    ...
    public static fromTime(days: number, hours: number, minutes: number, seconds: number, milliseconds: number): TimeSpan {
        return this.fromTimeStartingFromDays(days, hours, minutes, seconds, milliseconds);
    }

    private static fromTimeStartingFromDays(days: number, hours: number, minutes: number, seconds: number, milliseconds: number): TimeSpan | never {
    ...
    public toString(_includeMs: boolean = true): string {
        if (_includeMs) {
            if (this.days > 0)
                return `${this.days}.${this.hours.toString().padStart(2, "0")}:${this.minutes.toString().padStart(2, "0")}:${this.seconds.toString().padStart(2, "0")}.${this.milliseconds.toString().padStart(2, "0")}`;
            else
                return `${this.hours.toString().padStart(2, "0")}:${this.minutes.toString().padStart(2, "0")}:${this.seconds.toString().padStart(2, "0")}.${this.milliseconds.toString().padStart(2, "0")}`;
        }
        else {
            if (this.days > 0)
