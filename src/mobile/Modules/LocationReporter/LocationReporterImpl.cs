    ...
    var reportFlow = locationFlow
      ...
      .ScanAsync(ReportingCtx.Empty, async (_acc, _entry) =>
      {
        ...
        try
        {
          ...
          using var timedCts = new CancellationTokenSource(TimeSpan.FromSeconds(30));
          ...
          _discordIntegration.UpdatePresence(acc.SessionId.Value, location.Latitude, location.Longitude, prefs.RoomId, location.Speed, hr);
